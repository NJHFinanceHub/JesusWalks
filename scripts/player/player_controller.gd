...
func _build_collider_and_mesh() -> void:
	...
	if get_node_or_null("BodyMesh") == null:
		...
		material.albedo_color = Color(0.87, 0.84, 0.76)
...
func _handle_movement(delta: float) -> void:
	...
	if _dodge_timer > 0.0:
		...
		if _lock_target != null and is_instance_valid(_lock_target) and not _lock_target.is_redeemed():
			var lock_direction := _lock_target.global_position - global_position
...
func _update_camera(delta: float) -> void:
	...
	if _lock_target != null and is_instance_valid(_lock_target) and not _lock_target.is_redeemed():
		var to_target := (_lock_target.global_position + Vector3(0.0, 1.1, 0.0)) - _camera_rig.global_position
		var desired_yaw := atan2(to_target.x, to_target.z)
		var flat_distance := max(Vector2(to_target.x, to_target.z).length(), 0.05)
		var desired_pitch := clamp(-atan2(to_target.y, flat_distance) - 0.08, -0.7, 0.35)
...
func _regen_stamina(delta: float) -> void:
	...
	current_stamina = min(current_stamina + stamina_regeneration * delta, max_stamina)
...
func _try_healing_miracle() -> void:
	...
	if mesh != null and mesh.material_override is StandardMaterial3D:
		...
		highlight.emission = Color(0.92, 0.83, 0.55)
		highlight.emission_energy_multiplier = 1.4
		mesh.material_override = highlight


func receive_enemy_attack(attacker: EnemyAI, damage: float, posture_damage: float) -> void:
	...
	if _is_blocking and current_stamina > 0.0:
		var stamina_loss := posture_damage * 1.2
		...
		if current_stamina <= 0.01 and _perfect_block_timer <= 0.0:
			_apply_health_damage(damage * 0.35)
		return

	_apply_health_damage(damage)
...
func _handle_defeat() -> void:
	...
	var closest_site: PrayerSite = prayer_sites[0] as PrayerSite
	var closest_distance := global_position.distance_squared_to(closest_site.global_position)
	for node in prayer_sites:
		var site := node as PrayerSite
		if site == null:
			continue
		var test_distance := global_position.distance_squared_to(site.global_position)
...
func rest_at_prayer_site(site: PrayerSite) -> void:
	if site == null:
		return
...
func set_active_prayer_site(site: PrayerSite) -> void:
	_active_prayer_site = site
	set_context_hint(site.get_prompt_message())


func clear_active_prayer_site(site: PrayerSite) -> void:
...
func save_to_slot(slot_id: int) -> bool:
	...
	var ok := SaveSystemScript.save_slot(slot_id, payload)
	if ok:
...
func load_from_slot(slot_id: int) -> bool:
	...
	var ok := _apply_save_payload(payload)
	if ok:
...
func _apply_save_payload(payload: Dictionary) -> bool:
	...
	var world_data := payload.get("world", {})
	if world_data is Dictionary:
		var world_dict := world_data as Dictionary
		var enemy_states := world_dict.get("enemies", {})
		if enemy_states is Dictionary:
			var enemy_dict := enemy_states as Dictionary
			for node in get_tree().get_nodes_in_group("enemies"):
				var enemy := node as EnemyAI
				if enemy == null:
					continue
...
func _find_attack_target(max_distance: float, arc_degrees: float) -> EnemyAI:
	if _lock_target != null and is_instance_valid(_lock_target) and not _lock_target.is_redeemed():
		if global_position.distance_to(_lock_target.global_position) <= max_distance + 0.7:
			return _lock_target

	var best_target: EnemyAI
	...
	for node in get_tree().get_nodes_in_group("enemies"):
		...
		if distance < 0.1:
			continue
		var angle := rad_to_deg(acos(clamp(forward.normalized().dot(to_enemy.normalized()), -1.0, 1.0)))
